...
from .datagen.funs_ref import di_ref
...
class SurvLoader:
    ...
    Attributes:
        ...
        df_ds:      pd.DataFrame with columns:
                        ds (dataset alias)
                        is_td (has time-varying features)
                        n (sample size)
                        n_fac (number of categorical columns)
                        n_ohe (not of one-hot-encoded columns)
                        n_num (number of continuous columns)
    """

    def __init__(self):
        # Link to dataset source
        dir_SurvSet = Path(__file__).parent
        fold_datagen = os.path.join(dir_SurvSet, 'datagen')
    ...
    def load_dataset(self, ds_name):
        ...
        Returns:            
            ...
            df has the following columns:
                pid (unique ID)
                time (time to event or start time)
                time2 (None or end tend)
                num_{} (numeric columns)
                fac_{} (categorical columns)

            ref is the url which contains the feature description found from the original source
        """
        assert ds_name in self.df_ds[
            'ds'].to_list(), (
                '%s not found in dataset directory! See SurvLoader.df_ds["ds"] for a list of valid datasets' % ds_name)
