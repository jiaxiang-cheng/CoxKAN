...
from api.baseline.nsc.nsc_torch import NeuralSurvivalClusterTorch
import api.baseline.nsc.losses as losses
from api.baseline.nsc.utilities import train_nsc
...
class NeuralSurvivalCluster(DeepSurvivalMachines):
    """
    Model API to call for using the method
    Preprocess data to shape it to the right format and handle CUDA
    """

    def __init__(self, inputdim, k, cuda=torch.cuda.is_available(), **params):
        super().__init__(inputdim, k)
    ...
    def fit(self, x, t, e, vsize=0.15, val_data=None, optimizer="Adam", random_state=100, **args):
        """
        ...
        model = train_nsc(model, x_train, t_train, e_train, x_val, t_val, e_val, cuda=self.cuda == 2)
