...
class NeuralSurvivalClusterTorch(nn.Module):
    ...
    def __init__(self, inputdim, layers=None, act='ReLU6', layers_surv=None, representation=50,
        ...
        self.outcome = nn.ModuleList(
            [create_representation_positive(1 + self.representation, layers_surv + [risks], act_surv, self.dropout) for
             _ in range(self.k)])  # Model P(survival | risk, cluster)
        self.competing = nn.ParameterList([nn.Parameter(torch.randn(risks)) for _ in range(
            self.k)])  # Proba for the given risk P(risk | cluster) - Fixed for a cluster
    ...
    def forward(self, x, horizon):
        ...
        # For each cluster
        log_sr, log_beta = [], []
        tau_outcome = [horizon.clone().detach().requires_grad_(True).unsqueeze(1) for _ in
                       range(self.k)]  # Requires independent clusters
        ...
        for outcome, latent, balance, tau in zip(self.outcome, self.latent, self.competing, tau_outcome):
            ...
            log_beta.append(self.softlog(balance).unsqueeze(0).repeat(len(x), 1).unsqueeze(
                -1))  # Balance between risks in this cluster (fixed for the cluster)
    ...
    def gradient(self, outcomes, horizon, e):
        ...
        return torch.cat(grad([- outcomes[:, risk][e == (risk + 1)].sum() for risk in range(self.risks)], horizon,
                              create_graph=True), 1).clamp_(1e-10)
