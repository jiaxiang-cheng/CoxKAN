...
class Imputer:
    ...
    def fit_transform(self, data, cat_feats, num_feats, fill_value=-1, n_neighbors=5, **kwargs):
        ...
        **kwargs
        ...
        return self.fit(data, cat_feats=cat_feats, num_feats=num_feats, ill_value=fill_value).transform(data)
...
class Preprocessor:
    ...
    def transform(self, data):
        ...
        if self.one_hot:
            ...
            data_transformed = pd.get_dummies(data_transformed, dummy_na=False, drop_first=True, dtype='int')
    ...
    def fit_transform(self, data, cat_feats, num_feats, fill_value=-1, n_neighbors=5, **kwargs):
        ...
        **kwargs
        ...
        imputer_output = self.imputer.fit_transform(
            data, cat_feats=cat_feats, num_feats=num_feats, fill_value=fill_value, n_neighbors=n_neighbors, **kwargs)
        output = self.scaler.fit_transform(imputer_output, num_feats=num_feats)

        if self.one_hot:
            output[cat_feats] = output[cat_feats].astype('category')
            output = pd.get_dummies(output, dummy_na=False, drop_first=True, dtype='int')
